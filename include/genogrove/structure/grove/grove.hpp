...
class grove {
  public:
    ...
    void insert_data(std::string_view index, key_type ktype, data_type dtype)
        ...
        gdt::key<key_type, data_type> key(ktype, dtype);
        insert(index, key);
    }
    ...
    gdt::query_result<key_type, data_type> intersect(query_type& query) {
    ...
    void search_iter(node<key_type, data_type>* node, key_type& query, gdt::query_result<key_type, data_type>& result) {
        if(node == nullptr) {
            return;
        }
        if(node->get_is_leaf()) {
