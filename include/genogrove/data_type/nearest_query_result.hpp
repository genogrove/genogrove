...
#ifndef GENOGROVE_DATA_TYPE_NEAREST_QUERY_RESULT_HPP
#define GENOGROVE_DATA_TYPE_NEAREST_QUERY_RESULT_HPP
...
namespace genogrove::data_type {
    ...
    /**
     * @brief Result of a nearest-neighbor query — the predecessor and successor
     *        of a query in the grove's sort order, restricted to non-overlapping keys.
     *
     * Returned by grove::nearest_neighbors(). Either field may be null:
     ...
     * closest non-overlapping key to the left and right of the query, respectively.
     * For scalar key types (numeric, kmer), it is the closest key by sort order on
     * either side, excluding any key that satisfies overlaps() with the query.
     *
     * Distance to a returned neighbor is type-specific and computed by the caller
     * from the key values (e.g., `query.start - predecessor.end - 1` for closed-coord
     ...
     *
     * @see grove::nearest_neighbors()
     * @see query_result for the overlap-query result container
     */
    template <typename key_type, typename data_type = void>
    class nearest_query_result {
        public:
            /**
             * @brief Default-construct with both neighbors null.
             */
            nearest_query_result() = default;
...
#endif //GENOGROVE_DATA_TYPE_NEAREST_QUERY_RESULT_HPP