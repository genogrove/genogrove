...
- **Sorted-insert hot-path regression from #305**: replacing the sorted-leaf split with the symmetric `split_mid() = order/2` made leaf splits fire roughly twice as often on sorted append, causing a ~2× runtime regression at small orders (1.73× at `order=5, N=100` up to 2.07× at `order=5, N=1000` on the gh-pages continuous benchmark chart). Reintroduce an asymmetric leaf split (`mid = order - 1`) only on the sorted-insert path via `split_leaf_node_sorted()`; regular insert keeps the symmetric split. Internal splits stay symmetric regardless, so the #306 degenerate `(0 keys, 1 child)` right internal cannot reappear. The validator (`validate_tree_invariants` in `key_type_grove_test.hpp` and the local validator in `grove_remove_test.cpp`) now exempts the rightmost leaf from the min-occupancy check, matching standard right-biased B+ tree behavior. ([#308](https://github.com/genogrove/genogrove/pull/308))
