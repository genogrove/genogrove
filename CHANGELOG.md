...
### Refactored
- **Replace magic numbers with named constants**: `interval::INVALID_POSITION` replaces `std::string::npos` sentinel, `kmer::BASE_MASK` for 2-bit nucleotide masking, named compression magic byte arrays in `filetype_detector`, `MAX_BED_SCORE`/`MAX_RGB_VALUE`/`MAX_GFF_PHASE` validation constants, `DEFAULT_TREE_ORDER` in CLI, and simplified grove split formula. ([#187](https://github.com/genogrove/genogrove/issues/187), [#260](https://github.com/genogrove/genogrove/pull/260))
...
- **Add `[[nodiscard]]` to non-trivial pure query methods**: Applied to `intersect()`, `calc_parent_key()`, `get_neighbors()`, `get_edges()`, `get_neighbors_if()`, and `has_next()` — methods where discarding the result wastes resources. Updated `CONTRIBUTING.md` with revised policy. ([#150](https://github.com/genogrove/genogrove/issues/150), [#266](https://github.com/genogrove/genogrove/pull/266))
- **Remove dead `node::set_keys()` and `node::set_children()`**: Neither method was called anywhere. Both bypassed node's ownership semantics. ([#237](https://github.com/genogrove/genogrove/issues/237), [#267](https://github.com/genogrove/genogrove/pull/267))
- **Decompose `grove.hpp` into `.ipp` files**: Split 1351-line file into `grove_graph.ipp`, `grove_insert.ipp`, `grove_query.ipp`, `grove_serialize.ipp`. Also decomposed `split_node()` into `split_leaf_node()` + `split_internal_node()`. ([#184](https://github.com/genogrove/genogrove/issues/184), [#280](https://github.com/genogrove/genogrove/pull/280))
...
- **Defensive checks**: Check `newlocale()` return value before `strtod_l` in `gff_reader::parse_score`, add empty-keys precondition to `node::calc_parent_key()`, and remove dead `filetype_detector::validate_extension` declaration. ([#270](https://github.com/genogrove/genogrove/issues/270), [#271](https://github.com/genogrove/genogrove/issues/271), [#272](https://github.com/genogrove/genogrove/issues/272), [#279](https://github.com/genogrove/genogrove/pull/279))
- **CLI handlers called `exit(1)` bypassing RAII cleanup**: Removed try/catch blocks with `exit(1)` from BED and GFF handlers — exceptions now propagate to `main()` which already catches and reports them. ([#268](https://github.com/genogrove/genogrove/issues/268), [#287](https://github.com/genogrove/genogrove/pull/287))
- **zlib streambuf classes missing explicit move deletion**: `deflate_streambuf` and `inflate_streambuf` could be implicitly moved, leaving dangling `z_stream` handles. Explicitly deleted move operations. ([#269](https://github.com/genogrove/genogrove/issues/269), [#288](https://github.com/genogrove/genogrove/pull/288))
- **Leaf chain traversal in `search_iter` could overflow the stack and miss overlapping intervals**: Converted recursive sibling chain walk to iteration. Fixed leaf pruning condition — checking `first_key.end < query.start` is unsafe with (start, end) sort order; now only prunes when `first_key.start > query.end`. ([#274](https://github.com/genogrove/genogrove/issues/274), [#290](https://github.com/genogrove/genogrove/pull/290))
- **Interval and genomic_coordinate setters bypass validation**: Replaced `set_start()`/`set_end()` with atomic `set_range(start, end)` that validates both positions together. Added strand validation to `genomic_coordinate::set_strand()`. ([#273](https://github.com/genogrove/genogrove/issues/273), [#289](https://github.com/genogrove/genogrove/pull/289))
- **BED reader uses locale-dependent `stoi`/`stoul`**: Replaced with `std::from_chars` in `parse_score` and `parse_thickness` for locale-independent parsing consistent with the rest of the codebase. ([#275](https://github.com/genogrove/genogrove/issues/275), [#291](https://github.com/genogrove/genogrove/pull/291))
- **`vertex_count()` and `indexed_vertex_count()` inflated by separator keys**: Both used `key_storage.size()` which includes internal B+ tree separator keys. Added `leaf_key_count` counter that tracks only data insertions. ([#282](https://github.com/genogrove/genogrove/issues/282), [#293](https://github.com/genogrove/genogrove/pull/293))
- **Dangling pointer in bulk insert when `build_tree_bottom_up` throws**: Old root was deleted before build completed. Now RAII-guards with `unique_ptr` and erases map entries first. ([#283](https://github.com/genogrove/genogrove/issues/283), [#294](https://github.com/genogrove/genogrove/pull/294))
- **Internal split could produce sibling with zero keys**: For order=3, `default_mid()` and `sorted_mid()` both return 2, leaving the right sibling empty after an internal split. Now clamps mid to `[1, keys.size()-2]`. ([#285](https://github.com/genogrove/genogrove/issues/285), [#295](https://github.com/genogrove/genogrove/pull/295))

### Changed
- **Fix Doxygen doc comments and standardize SPDX license identifiers**: Changed `graph_overlay.hpp` doc comments from `/*` to `/**` for Doxygen. Standardized all SPDX identifiers to `GPL-3.0-or-later` (was mixed MIT/GPLv3). Fixed typo in `ranges.hpp`. ([#276](https://github.com/genogrove/genogrove/issues/276), [#277](https://github.com/genogrove/genogrove/issues/277), [#278](https://github.com/genogrove/genogrove/pull/278))
