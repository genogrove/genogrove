...
### Added
- **Removal coverage for unsorted-built trees**: the removal suite built every tree with the `sorted` tag, so borrow, merge and root collapse were only exercised against sorted-append shapes. Adds a randomized case that builds unsorted and removes all keys in random order at orders 3, 4 and 6, validating structure and survivor reachability as the tree drains. ([#519](https://github.com/genogrove/genogrove/issues/519), [#527](https://github.com/genogrove/genogrove/pull/527))
