...
### Refactored
- **Dropped `[[nodiscard]]` from `registry::deserialize`**: the method mutates the singleton in place (that side effect is its purpose) and returns a reference only as a convenience, so callers legitimately ignore it — matching the project's nodiscard policy for side-effecting functions with incidental returns. Removes the `-Wunused-result` warnings it produced across `registry_test.cpp`. `grove::deserialize` keeps the attribute (it returns a fresh grove by value). ([#466](https://github.com/genogrove/genogrove/pull/466))
