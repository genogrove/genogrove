...
- **Inconsistent throw-on-null across the reverse-traversal API — breaking change**: `graph_overlay`'s `get_neighbors_if`, `get_in_neighbors_if`, `get_edge_list`, and `get_in_edge_list` had no null check at all (silently returned empty via `incident.find(nullptr)`), unlike `get_neighbors`/`get_in_neighbors` (always threw) and `grove_view`'s equivalents of all four (already threw). Added the same `std::invalid_argument` null check to all four, aligning `graph_overlay`/`grove` with `grove_view`'s convention. **Any caller passing null to these four `graph_overlay`/`grove` methods and relying on an empty-result fallback will now throw instead** — no production code or existing test relied on the old behavior. ([#541](https://github.com/genogrove/genogrove/issues/541), [#550](https://github.com/genogrove/genogrove/pull/550))
- **`reorder_incoming` could still corrupt outgoing order for an interleaved self-loop**: the position-preserving-overwrite fix in #545 relocated a self-loop's shared `incident[target]` slot correctly only when the self-loop wasn't interleaved with another source targeting the same key — a self-loop occupies one slot serving both directions at once, so overwrite alone can't satisfy two independent positional constraints. `reorder_incoming` now merges two already-correct sequences — outgoing order (untouched by replay) and the desired incoming order — using self-loops as sync points, since they're the only entries present in both and their relative order is identical in each. ([#546](https://github.com/genogrove/genogrove/issues/546), [#551](https://github.com/genogrove/genogrove/pull/551))
...
### Fixed
- **Unsorted `insert()` misrouted keys in trees deeper than two levels**: insertion descended by comparing the key against each child's separator, but separators are subtree bounding boxes — for interval-like key types a box's start is the subtree *minimum*, so the test asked "does this key start after the child's leftmost key" when routing needs "after its rightmost key". Nearly every key answered yes, so an out-of-order key was pushed one child too far right at every level, and no later split moved it back (a split subdivides a position in the leaf chain, it never relocates a key). The key stayed present and sorted *within* its leaf and every per-node invariant still held; what broke was the global order across leaves, which `search_overlaps` depends on — it descends to one leaf, walks the chain forward, and stops once a leaf starts past the query, leaving anything stranded behind that point unreachable, including by a query for its own coordinates (shuffled inserts at order 3 lost 1942 of 2000 point queries; at order 100 the tree stayed shallow enough to hide it). `insert_sorted`, bulk build and all query paths were unaffected — a key greater than everything belongs in the rightmost child either way, which is why every hot path masked the bug. Routing now uses each child's **subtree maximum**, the actual B+ tree separator: every node caches its own maximum (leaf: last key; internal: last child's cached maximum, both O(1)), refreshed bottom-up by every path that changes structure — insert, both splits, sorted append, root promotion, bulk build, removal, borrow, merge — and rebuilt on deserialize, where it is derived state rather than serialized (no `.gg` format change). `find_leaf()` routes identically so removal looks in the subtree insertion used, and the scalar special case is gone: with a real maximum available one comparison is correct for every key type. Separately, both split paths left the pre-split node's separator on the right half, where its start belongs to the left half (58 such boxes in a 500-key order-3 tree) — harmless for routing, which no longer reads separators, but it made the separator sequence non-monotonic and loosened query pruning, so both splits now narrow it to the right half's own range. The shared test validator gains the three invariants that would have caught this — the concatenated leaf sequence is globally sorted, each separator equals its child's real subtree range (computed from leaf keys, so a stale separator cannot validate against itself), and each cached maximum matches its subtree — with the internal-node ordering check relaxed to compare starts for interval-like types, since a box's end is a subtree maximum and aggregating mixed strands yields the `'*'` wildcard, which `operator<` sorts before any concrete strand. New coverage: a typed deep-shuffled-insert test asserting the leaf chain equals the sorted key sequence for every key type (the first coverage of scalar routing at depth), plus interval-specific sibling-boundary shapes — equal starts, nesting, duplicates, touching, one spanning interval — across orders 3/4/10/100, the two descent extremes, and mixed-strand coordinates. The cached maximum is stored as a pointer into the grove's key storage rather than a copy, and `is_leaf` is packed next to `order`, so `sizeof(node)` is unchanged — holding it by value grew the node from 80 to 104 bytes, which at small orders (thousands of tiny nodes) slowed every insert path by up to 1.8x; `compact()` rebuilds the caches since it replaces the storage they point into. ([#517](https://github.com/genogrove/genogrove/issues/517), [#518](https://github.com/genogrove/genogrove/pull/518), [#522](https://github.com/genogrove/genogrove/pull/522))
...
### Changed
- **`registry` lock-free read contract restated: undefined behavior under concurrent writers**: `registry`'s unlocked fast paths (`get`/`contains`/`size`/`empty`) read the `std::deque` + `std::unordered_map` that `intern`/`clear`/`deserialize` mutate under the mutex. The docs called these "best-effort snapshots," implying a defined-but-stale value; an unsynchronized read concurrent with `push_back`/move-assign is in fact a data race (undefined behavior). The Doxygen now states plainly that these reads are safe only when no writer runs concurrently. Documentation only — no code or behavior change; the library is single-threaded today (see [#86](https://github.com/genogrove/genogrove/issues/86)), and locking/atomics were considered and deferred as premature. ([#487](https://github.com/genogrove/genogrove/issues/487), [#515](https://github.com/genogrove/genogrove/pull/515))

### Fixed
- **Bulk builds above `INT_MAX` keys no longer silently corrupt (`distribute_evenly` size_t fix)**: the bottom-up bulk-build helper computed its group count as `ceil_div(static_cast<int>(total), max_per_group)`, narrowing the `size_t` key count to `int`. A single-index bulk insert of more than `INT_MAX` (~2.1 billion) keys overflowed the cast to a garbage `num_groups`, silently corrupting the leaf/parent distribution — plausible at the ~100 GB genome-scale target. `even_distribution` and `distribute_evenly` were extracted from `grove`'s private body into `structure::detail` (they use no grove state) and now compute entirely in `size_t` with an overflow-safe ceiling; the extraction also makes the helper unit-testable above `INT_MAX` without allocating billions of keys. No behavior change below `INT_MAX`; no public API or format change. ([#486](https://github.com/genogrove/genogrove/issues/486), [#512](https://github.com/genogrove/genogrove/pull/512))
- **Bounded file-controlled allocation counts when reading a serialized `.gg` (untrusted-input DoS)**: several deserialization paths read a size/count straight from the file and allocated against it before the backing data was read, with no bound, so a tiny corrupt or malicious `.gg` — reachable via `isec -i file.gg` or any shared index — could force a multi-GB allocation and OOM (and pre-`try` reserves escaped as `std::bad_alloc` rather than the library's `runtime_error`). Every file-controlled count is now bounded before it drives an allocation, throwing `std::runtime_error` like other malformed-stream paths: a new `detail::require_backing_bytes` rejects counts the remaining (seekable) stream cannot back — applied to the index count, index name lengths, block count, and per-block compressed length in both `grove::deserialize` and `grove_view`; `serialization_traits<std::string>` bounds the claimed length before allocating; `block_inflater::decompress` gains a 1 GiB output cap (decompression-bomb guard); external-block key counts are rejected above the writer's chunk size (`detail::max_external_keys_per_block`) with per-iteration stream checks in both readers; `memory_streambuf` became seekable (and its `seekoff` validates in offset space, avoiding out-of-range pointer UB); and `bed_reader::read_size_vector` caps the BED12 block count, mirroring `gff_reader`'s existing guard. No public API or `.gg` format change — valid indexes read identically. ([#484](https://github.com/genogrove/genogrove/issues/484), [#511](https://github.com/genogrove/genogrove/pull/511))
...
- **Test coverage for cpp-audit-surfaced gaps**: direct unit tests for `links::apply_to_grove` (asserts the `edges_added` count with exact-duplicate collapsing and parallel-metadata edges kept, and the missing-name `std::runtime_error`), previously exercised only indirectly via subprocess e2e; plus CLI e2e negatives — an unsupported (FASTA) query is rejected, a non-`.gg` file passed to `isec -i` fails the header magic check cleanly, and `idx` on an empty (0-record) input produces a deserializable `.gg` (round-tripped via `isec -i`). Test-only; no source change. ([#493](https://github.com/genogrove/genogrove/issues/493), [#498](https://github.com/genogrove/genogrove/pull/498))

### Refactored
- **Deduplicated the CLI `idx`/`isec` dispatch tails**: `index::execute`'s BED and GFF branches shared the whole open-output → write-header → `serialize` → post-write-check sequence, differing only in grove type and payload enum — extracted into `write_index<grove_t>(grove, outputfile, payload_type)` called from both. `intersect::execute`'s index path had a 4-way dispatch (BED/GFF × eager/`--in-place`) that varied only by payload type and printer — extracted into `query_index<payload_t>(...)` which makes the `grove_view` (in-place) vs `deserialize` (eager) choice once, collapsing four hand-written blocks to two payload branches over one helper. Behavior-preserving (verified end-to-end: `idx` + `isec` eager/`--in-place`/`-t` on both BED and GFF); the in-memory `-t` target path is left as-is, having no eager/in-place split to unify. ([#491](https://github.com/genogrove/genogrove/issues/491), [#502](https://github.com/genogrove/genogrove/pull/502))
- **Constraint scope, const-correctness, and `[[nodiscard]]` consistency**: API-shape polish with no behavior change. `key::set_data`/`get_data` now gate their `requires` on `data_t` rather than the deduced argument, so on a dataless `key<T, void>` they are cleanly SFINAE-removed instead of hard-erroring in the `std::monostate` member; the single-arg `key(key_t)` constructor is constrained on `data_t` being default-constructible (or void), matching the default ctor. `node::calc_keys_aggregate`/`calc_subtree_range` become `const`, and `grove::insert`/`insert_iter`/`insert_sorted` take the key by `const&`. `[[nodiscard]]` is applied to the pure getters across `node`, `grove`, and the data_type result types (`interval::to_string`, `key::to_string`, `query_result`, `flanking_query_result`, `serializer::read`), matching `graph_overlay`'s existing policy. Adds a `static_assert` test that the data accessors are removed on a dataless key and present on a data-carrying one. ([#488](https://github.com/genogrove/genogrove/issues/488), [#499](https://github.com/genogrove/genogrove/pull/499))
- **Removed the orphaned `deflate_streambuf`/`inflate_streambuf`**: grove serialization moved to the one-shot block codecs (`block_deflater`/`block_inflater`, format 0.2), leaving the streaming streambufs with no production callers (only test references), a stale class docstring implying they were still the serializer, and throwing streambuf virtuals. Deleted both classes (182 lines) and their now-unused `<istream>`/`<ostream>` includes; kept `block_deflater`/`block_inflater`/`memory_streambuf`. The `z_stream` double-free guard (non-copyable/non-movable `static_assert`s) is redirected onto the live block codecs, and the `#301` serialization regression test keeps its clean-stream check (comment reworded). ([#489](https://github.com/genogrove/genogrove/issues/489), [#500](https://github.com/genogrove/genogrove/pull/500))

### Fixed
- **Bulk sorted-append into a non-empty index left an unbounded internal node**: appending pre-sorted keys into an already-populated index (`insert_data(index, data, sorted, bulk)` on a non-empty tree — the rightmost-append path) handled a leaf overflow with a single-level split that split the overflowing leaf's parent but never re-checked whether that parent itself then overflowed. `insert_sorted()` handled the same case with a `while`-loop that cascades to the root. Over enough appends one internal node grew without bound — queries still returned every key (`intersect` scans all children regardless of count), so it never threw or segfaulted, but the B+ tree invariant "no node exceeds `order-1` keys" was violated and that node degraded to O(n). The empty-index bulk build (bottom-up) was always correct. Fixed by extracting the correct cascade into a shared `cascade_split_sorted_append()` used by both sorted-append sites, and the identical root-promotion step into `promote_new_root()` shared by all three insert paths, so the three near-duplicate copies `/cpp-audit` flagged can no longer diverge. Adds a regression test that appends into a non-empty order-3 index and validates tree structure (fails pre-fix with a 249-key internal node). ([#490](https://github.com/genogrove/genogrove/issues/490), [#501](https://github.com/genogrove/genogrove/pull/501))
- **CLI fails cleanly on invalid arguments and honors `--version`**: cxxopts parsing (main-level and per-subcommand) ran outside any `try/catch`, and the subcommands don't `allow_unrecognised_options`, so a typo'd flag, a non-integer value (`-k foo`), or a missing option argument threw an uncaught exception and aborted via `std::terminate` instead of reporting an error and exiting 1. All parsing and dispatch now runs inside the existing `try/catch(std::exception)` (cxxopts exceptions derive from `std::exception`). The registered `-v,--version` flag was previously dead (it fell through to "No subcommand specified"); it now prints `genogrove <major>.<minor>.<patch>` and exits 0. Also corrects the `isec` `-q`/`-t`/`-o` help strings (copy-pasted from `idx`) and names the unused `compression` element in the three `detect_filetype` call sites. Adds e2e regressions for `--version` and the non-integer-`-k` clean-exit path. ([#485](https://github.com/genogrove/genogrove/issues/485), [#495](https://github.com/genogrove/genogrove/pull/495))
- **BED/GFF readers surface errors consistently**: two gaps closed. (1) The constructor's eager first-record validation threw on a malformed leading record even when `skip_invalid_lines` was set — violating the tolerance contract and diverging construction from iteration semantics; when the option is set the eager coordinate check is now skipped entirely and `read_next()`/`parse_line()` skips the bad first record like any other (empty-file/zero-record detection unchanged; `skip_invalid_lines == false` still throws at construction). (2) A BGZF/tabix I/O error escaped `read_next()` without populating `error_message` (unlike bam/vcf); `read_next()` now records `e.what()` in `error_message` before rethrowing, so a caller that catches the throw and calls `get_error_message()` sees it. Adds a `skipInvalidToleratesBadFirstRecord` regression per reader. ([#492](https://github.com/genogrove/genogrove/issues/492), [#497](https://github.com/genogrove/genogrove/pull/497))

## [0.25.2] - 2026-07-13

### Added
- **`grove_view` exposes edge payloads**: the partial (random-access) reader gains `get_edges(source)` and `get_neighbors_if(source, pred)`, mirroring the in-memory grove's adjacency surface, so a `.gg` can be walked *and* its edge metadata inspected without a full deserialize. The metadata was already parsed from the format-0.2 stream by `read_key_edges` and then discarded — it is now kept on `edge_ref` (zero-size via `[[no_unique_address]]` + `std::monostate` when `edge_data_type` is `void`, mirroring `graph_overlay::edge`). `get_edges` is `const` and returns an empty vector for a null source; `get_neighbors_if` resolves surviving targets' blocks on demand and throws on a null source, matching `get_neighbors`. Both are constrained to non-`void` `edge_data_type`. Read path only — no serialized-format change, no writer change; unblocks the pygenogrove `GroveView` bindings from surfacing edge metadata. ([#479](https://github.com/genogrove/genogrove/issues/479), [#480](https://github.com/genogrove/genogrove/pull/480))
...
- **`isec --in-place` — query a prebuilt index on disk**: a new opt-in flag on the `intersect` CLI subcommand reads a prebuilt `.gg` in place via `grove_view` (loading only the blocks each query touches) instead of deserializing the whole file into memory. Eager deserialize stays the default; `--in-place` requires `-i` (a `-t` target is built in memory and errors). `grove_view::open` gains a `data_offset` parameter so it can start reading after the CLI's 12-byte `gg_header`, and the bed/gff `grove_intersect` handlers are templated to serve both `grove<>` and `grove_view<>`. ([#463](https://github.com/genogrove/genogrove/issues/463), [#470](https://github.com/genogrove/genogrove/pull/470))
- **GFF/GTF support for `idx --links`**: the graph-overlay link feature now accepts GFF/GTF input via a new `--gff-name-tag <ATTR>` option that selects which attribute value identifies each record (e.g. `ID`, `gene_id`, `transcript_id`). Because GFF has no canonical name column, `--links` on GFF/GTF requires `--gff-name-tag`; every record must carry the attribute and its values must be unique across the file (missing or duplicate → error, mirroring the BED column-4 rule). On BED input the flag is ignored (BED links always match column 4). Internally the `name_to_key_map` type and `links::apply_to_grove` are templated on the payload (shared `handlers/name_map.hpp`) so BED and GFF share one link resolver. ([#441](https://github.com/genogrove/genogrove/issues/441), [#471](https://github.com/genogrove/genogrove/pull/471))
- **Per-edge metadata in `idx --links`**: the `--links` TSV accepts an optional 3rd column (`source<TAB>target<TAB>metadata`) that is attached to the graph edge as metadata; 2-column rows are unchanged. 1, 4+, or any empty column (including a present-but-empty 3rd) is rejected with the line number. The dedup key includes the metadata, so the same pair with two distinct metadata values yields two parallel edges while exact-duplicate rows collapse. **Format change:** `graph_overlay` metadata requires a non-`void` edge type, so the CLI groves move from `edge_data_type = void` to `std::string` across `idx`, `isec`, and `grove_view` — this changes the `.gg` on-disk format, so existing indexes must be regenerated (no serialization back-compat). ([#442](https://github.com/genogrove/genogrove/issues/442), [#472](https://github.com/genogrove/genogrove/pull/472))
- **Cross-type interval queries in `intersect`**: the query file type and the target/index type are now independent — any of BED/GFF/GTF may be queried against a BED or GFF target/index, in all four combinations. The output format follows the target payload type. Each handler's intersect is split into a query iterator (`for_each_bed_query` / `for_each_gff_query`) and a result printer (`print_bed_result` / `print_gff_result`), composed by a generic `run_intersect`; the four "query type must match" errors are removed. To make cross-type overlaps correct, both readers now normalize to a common 0-based-inclusive coordinate space — BED stays `interval(start, end-1)`, GFF becomes `interval(start-1, end-1)` at insert and query (same-type results and printed coordinates unchanged; the shifted GFF stored coords change the `.gg` numbers, so **regenerate GFF indexes** — no serialization back-compat). ([#439](https://github.com/genogrove/genogrove/issues/439), [#473](https://github.com/genogrove/genogrove/pull/473))
