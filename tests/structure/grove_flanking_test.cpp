...
TEST(GroveFlankingTest, EmptyGroveReturnsNullPair) {
    gst::grove<gdt::interval, int> g(8);
    auto nn = g.flanking(gdt::interval{100, 200}, "chr1");
    EXPECT_EQ(nn.get_predecessor(), nullptr);
    EXPECT_EQ(nn.get_successor(),   nullptr);
}

TEST(GroveFlankingTest, MissingIndexReturnsNullPair) {
    gst::grove<gdt::interval, int> g(8);
    g.insert_data("chr1", gdt::interval{50, 60}, 1, gst::sorted);
    auto nn = g.flanking(gdt::interval{100, 200}, "chr2");
    EXPECT_EQ(nn.get_predecessor(), nullptr);
    EXPECT_EQ(nn.get_successor(),   nullptr);
}

TEST(GroveFlankingTest, QueryBeforeAllKeysHasOnlySuccessor) {
    gst::grove<gdt::interval, int> g(8);
...
TEST(GroveFlankingTest, QueryAfterAllKeysHasOnlyPredecessor) {
    gst::grove<gdt::interval, int> g(8);
    auto* k = g.insert_data("chr1", gdt::interval{100, 200}, 1, gst::sorted);
    auto nn = g.flanking(gdt::interval{500, 600}, "chr1");
    EXPECT_EQ(nn.get_predecessor(), k);
    EXPECT_EQ(nn.get_successor(),   nullptr);
}

TEST(GroveFlankingTest, QueryBracketedByTwoKeys) {
    gst::grove<gdt::interval, int> g(8);
...
TEST(GroveFlankingTest, AbuttingKeysHaveZeroGapDistance) {
...
TEST(GroveFlankingTest, ChoosesClosestPredecessorAmongMany) {
    gst::grove<gdt::interval, int> g(8);
...
TEST(GroveFlankingTest, NestedIntervalsPickClosestEndForPredecessor) {
    ...
    auto nn = g.flanking(gdt::interval{200, 300}, "chr1");
    EXPECT_EQ(nn.get_predecessor(), outer);
}

TEST(GroveFlankingTest, MultiLeafTreePicksGlobalNearest) {
    ...
    auto nn = g.flanking(gdt::interval{440, 460}, "chr1");
...
TEST(GroveFlankingTest, StrandPredicateFiltersOutWrongStrand) {
    ...
    auto nn = g.flanking(gdt::genomic_coordinate{'+', 100, 200}, "chr1", same_strand);
    EXPECT_EQ(nn.get_predecessor(), far_match);
    EXPECT_EQ(nn.get_successor(),   nullptr);
}

TEST(GroveFlankingTest, WildcardQueryMatchesAnyStrand) {
    gst::grove<gdt::genomic_coordinate, int> g(8);
...
TEST(GroveFlankingTest, NumericPredecessorSuccessor) {
    ...
    auto nn = g.flanking(gdt::numeric{20}, "idx");
...
TEST(GroveFlankingTest, NumericQueryNotInTree) {
    gst::grove<gdt::numeric, int> g(8);
    auto* k1 = g.insert_data("idx", gdt::numeric{10}, 1, gst::sorted);
